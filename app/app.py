...
def main():
    ...
    parser.add_argument("--config_root",type=str,default=str(ROOT_DIR / "src/configs"),help="元のconfig. 前のコピーしたいときはこれを指定")
    parser.add_argument("--fps",type=int,default=4,help="描画のFPS")
    parser.add_argument("--saveto",type=str,default=str(ROOT_DIR / "history/data"),help="今回のconfigの保存先")
    args=parser.parse_args()


    config_root=Path(args.config_root)
    new_config_path=Path(args.saveto)/"configs"
